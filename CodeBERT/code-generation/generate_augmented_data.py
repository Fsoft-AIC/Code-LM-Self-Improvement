...
def main():
    ...
    # Calculating similarity with ground truth sequence
    # BLEU
    pseudo_data = []
    for k_best_list, ex in zip(k_best_lists, eval_examples):
        scores = [sentence_bleu.bleu([ex.target], pred)[0] for pred in k_best_list] 
        index = np.argmax(scores)
        y_hat = k_best_list[index]
        pseudo_data.append((ex.source, y_hat))

    objs = [{'code': code.strip(),
             'nl': source.strip()}
                    for (source, code) in pseudo_data]
    output_dir = os.path.join(args.output_dir, "bleu")
    ...
    with open(output_filepath, "w") as f:
        for obj in objs:
            json.dump(obj, f)
            f.write('\n')
    print("Saved pseudo_data.json into {}".format(output_dir))

    # CodeBLEU
    pseudo_data = []
    for k_best_list, ex in zip(k_best_lists, eval_examples):
        scores = []
        gold_fn = os.path.join(args.output_dir, "example.gold")
        output_fn = os.path.join(args.output_dir, "example.output")
        with open(gold_fn, "w", encoding="utf-8") as f:
            f.write(ex.target)
            f.write("\n")
        for pred in k_best_list:
            with open(output_fn, "w", encoding="utf-8") as f:
                f.write(pred)
                f.write("\n")
            codebleu = calc_code_bleu.get_codebleu(gold_fn, output_fn, "java", verbose=False)
    ...
    objs = [{'code': code.strip(),
             'nl': source.strip()}
                    for (source, code) in pseudo_data]
    output_dir = os.path.join(args.output_dir, "codebleu")
    os.makedirs(output_dir, exist_ok=True)
    output_filepath = os.path.join(output_dir, "pseudo_data.json")
    with open(output_filepath, "w") as f:
        for obj in objs:
