...
def main():
    ...
    checkpoint_path = os.path.join(args.output_dir, "checkpoint-best-bleu", "pytorch_model.bin")
    assert not os.path.exists(checkpoint_path), checkpoint_path
    # set log
    logging.basicConfig(format='%(asctime)s - %(levelname)s - %(name)s -   %(message)s',
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for idx, batch in bar:
                ...
                if len(losses) % args.gradient_accumulation_steps == 0:
                    ...
                    optimizer.zero_grad()
                    scheduler.step()
                    train_loss = round(np.mean(losses[-100*args.gradient_accumulation_steps:]),4)
            ...
            if args.do_eval:
                ...
                model.eval() 
                p=[]
                for batch in tqdm(eval_dataloader, total=len(eval_dataloader), desc="Eval bleu on dev set"):
                    batch = tuple(t.to(device) for t in batch)
                    source_ids = batch[0]                  
                    with torch.no_grad():
    ...
    if args.do_test:
        ...
        model.eval() 
        p=[]
        for batch in tqdm(eval_dataloader, total=len(eval_dataloader), desc="Eval bleu on test set"):
            batch = tuple(t.to(device) for t in batch)
            source_ids = batch[0]                  
            with torch.no_grad():
                preds = model(source_ids)   
